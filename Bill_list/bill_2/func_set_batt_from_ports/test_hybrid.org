#+TITLE: test_hybrid_batt_update and hybrid_main tests
#+TESTY: PREFIX="prob1"
...
#+TESTY: program='./test_hybrid_batt_update "set_batt_from_ports() mixed STATUS V"'
#+BEGIN_SRC sh
{
...
#+TESTY: program='./test_hybrid_batt_update "set_batt_from_ports() mixed STATUS P"'
#+BEGIN_SRC sh
{
    // Checks that only bit 4 of BATT_STATUS_PORT is used for
    // determining the Voltage/Percent mode. Bit 4 is 0 but other bits
...
#+TESTY: program='./test_hybrid_batt_update "set_display_from_batt() level 5 V"'
#+BEGIN_SRC sh
{
    // 90% is lowest percentage at which 5 bars will be shown
    BATT_VOLTAGE_PORT = -1;
...
#+TESTY: program='./test_hybrid_batt_update "set_display_from_batt() error"'
#+BEGIN_SRC sh
{
