...
protected:
  ...
  void ShowHT(GenListNode<ElemType> *hd, bool a) const;
  // 显示以hd为头结点的广义表
  ...
  // 广义表
  void CodeReverse(const GenListNode<ElemType> *&Head); // 将结点的链表给倒置一下
...
public:
  ...
  void Insert(GenListNode<ElemType> *&Head);
  // 将子表subList作为表头加入到广义表最前面
  Status Insert(int i,
                const ElemType &e); // 将原子元素e作为表头加入到广义表指定位置
  Status Insert(int i, GenList<ElemType> &subList); // 将子表插入到广义表指定位置
  Status Delete(int i);                 // 删除广义表中第i个元素
  GenList<ElemType> GetHead();          // 返回广义表表头
  Status GetTail(GenList<ElemType> &g); // 返回广义表表头
  int GetDepth();                       // 计算广义表深度
  int GetLength() const;                // 求广义表的长度
  ...
  void ShowHT(bool a); // 显示广义表的表头和表尾
  void Reverse(GenListNode<ElemType> *&head); // 将广义表倒置
};
...
template <class ElemType>
GenListNode<ElemType> *&GenList<ElemType>::Gethead()
// 操作结果：返回广义表的第一个元素
{
  return head;
}
...
template <class ElemType>
Status GenList<ElemType>::Insert(int i, const ElemType &e)
// 操作结果：将原子元素e作为表头加入到广义表指定位置
{
  if (i < 1 || i > (GetLength() + 1))
    ...
    for (int k = 1; k < i;
         k++) { //  使p和pre分别指向插入位置的前驱节点和后驱结点
      pre = p;
      p = p->tLink;
    }
    GenListNode<ElemType> *temp = new GenListNode<ElemType>(ATOM, p);
    temp->atom = e;
    pre->tLink = temp;
    return SUCCESS;
  }
...
template <class ElemType>
void GenList<ElemType>::Insert(GenListNode<ElemType> *&LHead) {
  GenListNode<ElemType> *p = LHead->tLink;
  GenListNode<ElemType> *q, *temp = head;
  for (; p != NULL; p = p->tLink) {
    if (p->tag == ATOM) {
      q = new GenListNode<ElemType>(ATOM, temp->tLink);
      q->atom = p->atom;
      temp->tLink = q;
      temp = temp->tLink;
      /*Show();
...
template <class ElemType>
GenList<ElemType> GenList<ElemType>::GetHead()
// 返回广义表的表头
{
  GenList<ElemType> g;
  if (head->tLink != NULL) {
    if (head->tLink->tag == LIST) {
      g.Insert(head->tLink->hLink);
      /*GenListNode<ElemType>* p = new GenListNode<ElemType>(LIST,
      g.head->tLink); p->hLink = head->tLink->hLink; g.head->tLink = p;*/
      /*int n = 1;
      GenListNode<ElemType>* p = head->tLink->hLink->tLink;
      GenListNode<ElemType>* temp,*temp1;
      temp1 = g.head->tLink;
      for (;p != NULL;p = p->tLink)
      {
              if (p->tag == ATOM)
      ...
      g.Show();*/
      return g;
    } else if (head->tLink->tag == ATOM) {
      GenListNode<ElemType> *p = new GenListNode<ElemType>(ATOM, g.head->tLink);
      p->atom = head->tLink->atom;
      g.head->tLink = p;
      return g;
    }
  } else {
...
template <class ElemType>
Status GenList<ElemType>::GetTail(GenList<ElemType> &g)
// 返回广义表的表头
{
  GenList<ElemType> temp = *this;
  if (head->tLink != NULL) {
    temp.Delete(1);
    g = temp;
    return SUCCESS;
...
template <class ElemType>
Status GenList<ElemType>::Insert(int i, GenList<ElemType> &subList)
// 操作结果：将子表subList作为表头加入到广义表最前面
{
  if (i < 1 || i > GetLength())
    return RANGE_ERROR; // 返回位置错
    ...
    GenListNode<ElemType> *temp = new GenListNode<ElemType>(LIST, p);
    temp->hLink = subList.head;
    pre->tLink = temp;
    subList.head->ref++;
...
// 操作结果：显示以hd为头结点的广义表
{
  bool frist = true;
  cout << "("; // 广义表以(开始
  for (GenListNode<ElemType> *p = hd->tLink; p != NULL;
       p = p->tLink) { // 依次处理广义表各元素
    if (frist)
      frist =
          false; // 第一个元素输出的时候不需要输出“，”，在输出后面的元素的时候
                 // 我们需要先输出一个“，”；
    else
      cout << ",";      // 不同元素这间用逗号隔开
    if (p->tag == ATOM) // 原子结点
...
template <class ElemType>
void GenList<ElemType>::ShowHT(GenListNode<ElemType> *hd, bool a) const
// 操作结果：显示以hd为头结点的广义表
{
  bool frist = true;
  if (!a) // 输出广义表表头
...
template <class ElemType>
void GenList<ElemType>::ShowHT(bool a)
// 操作结果：显示广义表
{
  ShowHT(head, a); // 调用辅助函数显示广义表
}
...
// 操作结果：释放以hd为表头的广义表结构
{
  ...
  if (hd->ref == 0) { // 引用数为0,释放结点所占用空间
    GenListNode<ElemType> *pre = hd, *p;           // 临时变量
    for (p = hd->tLink; p != NULL; p = p->tLink) { // 扫描广义表hd的顶层
      delete pre;                                  // 释放pre
      pre = p;
      if (p->tag == LIST)    // p为子表
...
template <class ElemType>
void GenList<ElemType>::CodeReverse(const GenListNode<ElemType> *&Head)
// 操作结果：输入广义表
{}

template <class ElemType>
void GenList<ElemType>::Reverse(GenListNode<ElemType> *&head)
// 将广义表倒置
{
  GenListNode<ElemType> *pre = head, *p = head->tLink, *temp = NULL;
  if (p != NULL) {
    for (; p->tLink != NULL;) {
      // pre->tLink = p->tLink;
      temp = p->tLink;
      p->tLink = temp->tLink;
      temp->tLink = pre->tLink;
      pre->tLink = temp;
    }
